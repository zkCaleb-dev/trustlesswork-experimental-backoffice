...
export type EscrowType = 'sr-v1' | 'sr-v2' | 'mr-v1' | 'mr-v2';
...
/** Deploy endpoint for an escrow type. */
export function escrowDeployPath(type: EscrowType): string {
  return TYPES.find((t) => t.value === type)!.path;
}
...
/** Prefill for edit mode; every field optional (falls back to create defaults). */
...
export interface EscrowFormSubmit {
  type: EscrowType;
  /** The escrow fields (no signer): deploy spreads them, update nests as `escrow`. */
  fields: Record<string, unknown>;
}

/**
 * Shared escrow form, used by both create (deploy) and edit (update). It owns
 * the field state, validation and field assembly, and emits a normalized
 * `{ type, fields }` payload; the caller wraps it into the right request
 * (deploy adds `signer`; update wraps `fields` as `escrow` + `admin`).
 */
export function EscrowForm({
  ...
  mode: 'create' | 'edit';
  initial?: EscrowFormInitial;
  submitting: boolean;
  submitLabel: string;
  submittingLabel: string;
  error?: string | null;
  onSubmit: (payload: EscrowFormSubmit) => void;
}) {
  const session = useSession();
  const myWallet = session.data?.wallet ?? '';

  const [type, setType] = useState<EscrowType>(initial?.type ?? 'sr-v2');
  ...
  const [platformFee, setPlatformFee] = useState(initial?.platformFee ?? '0');
  const [receiverMemo, setReceiverMemo] = useState(initial?.receiverMemo ?? '');

  const [trustlineContractId, setTrustlineContractId] = useState(
    initial?.trustlineContractId ?? '',
  );
  const [trustlineSymbol, setTrustlineSymbol] = useState(
    initial?.trustlineSymbol ?? '',
  );
  const [trustlineIssuer, setTrustlineIssuer] = useState(
    initial?.trustlineIssuer ?? '',
  );
  ...
  const [milestones, setMilestones] = useState<MilestoneInput[]>(
    initial?.milestones ?? [emptyMilestone()],
  );

  const [attempted, setAttempted] = useState(false);

  // Create-only: seed the creator into the operational roles as a convenience.
  useEffect(() => {
    if (mode !== 'create' || !myWallet) return;
  ...
  function submit(e: FormEvent) {
    e.preventDefault();
    setAttempted(true);
    if (errors.length === 0) onSubmit({ type, fields: buildFields() });
  }
  ...
  function collectErrors(): string[] {
    ...
    if (isV2) {
      ...
      if (
        ...
        errs.push('Admin must differ from every other role and the receiver.');
    } else {
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-6">
      ...
      <Section title="Basics">
        ...
        {isMulti && (
          <p className="text-xs text-muted-foreground">
            Multi-release: there&apos;s no single amount — each milestone carries
            its own amount and receiver below.
          </p>
        )}
      </Section>
      ...
      <Section title="Roles">
        ...
        {isMulti && (
          <p className="text-xs text-muted-foreground">
            Multi-release has no escrow-level receiver — the receiver is set per
            milestone.
          </p>
        )}
      </Section>

      <Section title={`Milestones${isV2 ? ' (optional)' : ''}`}>
        ...
        <Card className="border-destructive/30 bg-destructive/5">
          <CardContent className="flex flex-col gap-2">
            <div className="flex items-center gap-2 text-sm font-medium text-destructive">
              ...
              Fix {errors.length} {errors.length === 1 ? 'issue' : 'issues'} first
            </div>
